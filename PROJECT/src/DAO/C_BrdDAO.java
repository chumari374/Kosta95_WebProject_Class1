...
public class C_BrdDAO {
	...
	public List getC_BrdList(int page, int limit){
		...
		String getC_BrdList_sql = "select * from "
				+ "(select rownum rnum, NUM,EMPNO,SUBJECT,"
				+ "CONTENT,REF,DPTH,STEP,READNUM,TO_CHAR(WRITE_DATE, 'YYYY/MM/DD') AS WRITE_DATE from "
				+ "(select * from C_BRD order by REF desc,STEP asc)) "
				+ "where rnum>=? and rnum<=?";
