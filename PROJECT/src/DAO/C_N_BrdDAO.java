...
//		String getC_BrdList_sql = "select * from (select rownum rnum, NUM,EMPNO,TITLE,CONTENT,REF,DPTH,STEP,COUNT,WRITE_DATE,ENAME,NOTICE from (select * from C_BRD order by REF desc,STEP asc)) where rnum>=? and rnum<=?";
	...
	// 글 등록
	public boolean Insert(C_N_BrdDTO c_n_brd){
		...
		try {
			...
			sql = "insert into c_n_brd (NUM, EMPNO, TITLE, CONTENT, WRITE_DATE, ENAME , COUNT)"
					+ "values(Comp_notice_num.nextval,?,?,?,sysdate,?,0)";
			
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, c_n_brd.getEmpno());
			pstmt.setString(2, c_n_brd.getTitle());
			pstmt.setString(3, c_n_brd.getContent());
			pstmt.setString(4, c_n_brd.getEname());
...
//	// 글 수정
	public boolean Modify(C_N_BrdDTO modifyc_n_brd){
		
		String sql = "update c_n_brd set TITLE=?, CONTENT=? where NUM=?";
		
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, modifyc_n_brd.getTitle());
			pstmt.setString(2, modifyc_n_brd.getContent());
			pstmt.setInt(3, modifyc_n_brd.getNum());
			pstmt.executeUpdate();
			return true;
		} catch (Exception ex) {
	...
	// 글 삭제
		...
		public boolean Delete(int num) {
			...
			// ->답변 삭제 (x) 답변
			// ->답변_1
			String c_n_brd_delete_sql = "delete from c_n_brd where num=?";

			int result = 0;

			try {
				conn = ds.getConnection();
				pstmt = conn.prepareStatement(c_n_brd_delete_sql);
				pstmt.setInt(1, num);
				result = pstmt.executeUpdate();
				if (result == 0)
					return false;
		...
		public boolean isboardWriter(int num, int empno){
			...
			String board_sql = "select * from c_n_brd where NUM=?";
