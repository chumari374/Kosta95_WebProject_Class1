...
%>
	...
	<tr align=center height=20>
		<td colspan=7 style=font-family:Tahoma;font-size:10pt;>
			...
			<%}else{ %>
			<a href ="CompDataBoardList.cf?page=<%=level*5-5 %>"><input type="button" value="이전 " class="pageP"></a>
			<%} %>
			...
			<%for(int i=level*5-4;i<= end ;i++){
				...
				<%}else{ 
					if(i<=maxpage){
				%>
					<a href="CompDataBoardList.cf?page=<%=i %>"><input type="button" value="<%=i %>" class="page"></a>
				<%
					}
				 }
				%>
			...
			<%}else{ %>
			<a href="CompDataBoardList.cf?page=<%=level*5+1 %>"><input type="button" value="다음" class="pageN"></a>
			<%} %>
		</td>
	</tr>
	<!-- 페이징처리 끝 -->
